...
from gui_manager import show_message, OptionMultiSelector
...
if __name__ == "__main__":
    ...
    selector = OptionMultiSelector(selection_dates, "Select a date.")
    users_dates = selector.show()

    try:
        sess, tr, nu = get_last_session(max_num_of_sessions=6)

        for users_date in users_dates[::-1]:
            formatted_date = datetime.strptime(users_date, "%d.%m.%Y").strftime("%Y-%m-%d")
            result_dict.setdefault('DATE', []).append(formatted_date)
